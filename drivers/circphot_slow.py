...
- Downloads the primary spectral image MEF FITS (via the public S3 mirror),
  caches them on disk, and performs aperture photometry at the target pixel.
...
- bkgd_method = 'annulus': estimate a local background as the sigma‑clipped
  ...
  masking stars in the annulus via photutils DAOStarFinder.
...
Usage
  python circannphot_slow.py [options]
...
- --max-images N          Maximum images to process from the query (default: 200).

Examples
- Default (use cache if present; zodi subtraction):
    python circannphot_slow.py
- Force re-photometry from cached files with local-annulus background:
    python circannphot_slow.py --do-photometry --bkgd-method annulus \
        --annulus-r-in 6 --annulus-r-out 8

Notes
- Set SPHEREX_CACHE to control the on-disk cache root for FITS downloads.
